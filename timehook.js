...
// @name:en         TimerHooker Custom
// @namespace       https://gitee.com/HGJing/everthing-hook/
// @version         1.0.62
// @description     Control webpage timer speed | Accelerate skipping page timer ads | Video fast forward (slow motion) | Skip ads | Supports almost all web pages.
...
// @downloadURL https://update.greasyfork.org/scripts/372673/Timer%20Controller%7CVideo%20Ad%20Skip%7CVideo%20Ad%20Accelerator.user.js
// @updateURL https://update.greasyfork.org/scripts/372673/Timer%20Controller%7CVideo%20Ad%20Skip%7CVideo%20Ad%20Accelerator.meta.js
// ==/UserScript==
/**
 * ---------------------------
...
~function (global) {
        ...
        return {
		    ...
		    '    <div class="_th-item _item-100x">x100</div>\n' + // Added 100x button
                ...
                var clickMapper = {
                    ...
                    '_item-100x': function () {
                        changeTime(0, 0, false, false, 100); // Added 100x handler
                    }
                };
            ...
            applyGlobalAction: function (timer) {
                // Method for clicking the semicircle button on the interface
                timer.changeTime = function (anum, cnum, isa, isr, customMultiplier) {
                    if (isr) {
                    ...
                    var result;
                    if (!anum && !cnum && !customMultiplier) {
                        var t = prompt("Enter the desired timer change rate (current: " + 1 / timerContext._percentage + ")");
                        if (t == null) {
                            return;
                        }
                        ...
                        if (parseFloat(t) <= 0) {
                            alert("The rate cannot be less than or equal to 0");
                            timer.changeTime();
                            return;
                        }
                        result = 1 / parseFloat(t);
                    } else if (customMultiplier) {
                        result = 1 / customMultiplier;
                    } else {
                        if (isa && anum) {
            ...
            getHookedTimerFunction: function (type, timer) {
                ...
                return function () {
                    ...
                    // Storing the original time interval
                    var originMS = arguments[1];
                    // Getting the changed speed time interval
                    arguments[1] *= timerContext._percentage;
                    var resultId = timer.apply(window, arguments);
                    // Saving the ID and parameters obtained each time the timer is used
            ...
            percentageChangeHandler: function (percentage) {
                ...
                util.ergodicObject(timerContext, timerContext._timeoutIds, function (idObj, id) {
                    ...
                    // Resetting the next execution time
            ...
            hookDefineDetails: function (target, key, option) {
                ...
                if (target instanceof HTMLVideoElement && key === 'playbackRate') {
                    option.configurable = true;
                    console.warn('[Timer Hook]', 'Default video playback rate operation prevented');
    ...
    var generate = function () {
        return function (util) {
            ...
            var timerHooker = {
                // Used to store timer IDs and parameters
                _intervalIds: {},
                _timeoutIds: {},
                _auoUniqueId: 1,
                // Timer rate
                __percentage: 1.0,
                // Original methods before interception
                ...
                notifyExec: function (uniqueId) {
                    var _this = this;
                    if (uniqueId) {
                        // Clearing timeout stored records
                ...
                init: function () {
                    ...
                    // Setting the callback for percentage attribute modification
